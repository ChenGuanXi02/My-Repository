...
#
    ...
    def val_time_ensemble(self, model, test_data_loader, accelerator, thresholding=False, save_to=None):
        ...
        for data in tqdm(test_data_loader, disable=not accelerator.is_main_process):
            ...
            # === TTA 水平翻转增强开始 ===
            # 1. 正常看一遍原图
            ensem_out_origin = self.train_val_forward_fn(model, image=image, time_ensemble=True,
                                                         gt_sizes=[g.shape for g in gt], verbose=False)
            pred_origin = ensem_out_origin["pred"]

            # 2. 把原图水平翻转送进去看一遍 (image 是普通的 Tensor，直接 flip 没问题)
            image_flip = torch.flip(image, dims=[3])
            ensem_out_flip = self.train_val_forward_fn(model, image=image_flip, time_ensemble=True,
                                                       gt_sizes=[g.shape for g in gt], verbose=False)
            pred_flip_raw = ensem_out_flip["pred"]

            # 3. 极其关键：兼容 List 格式的 Mask 翻转与融合
            if isinstance(pred_flip_raw, list):
                # 针对每张尺寸不同的图，挨个取出 Tensor 进行翻转
                pred_flip_back = [torch.flip(p, dims=[-1]) for p in pred_flip_raw]
                # 挨个取出 Tensor 求平均
                ensem_res = [(p_orig + p_back) / 2.0 for p_orig, p_back in zip(pred_origin, pred_flip_back)]
            else:
                # 兜底：如果以后变成了纯 Tensor，按老规矩走
                pred_flip_back = torch.flip(pred_flip_raw, dims=[-1])
                ensem_res = (pred_origin + pred_flip_back) / 2.0
            # === TTA 水平翻转增强结束 ===
