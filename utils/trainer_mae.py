...
#
    ...
    #         image = image.to(device).squeeze(1)
    #         out = self.train_val_forward_fn(model, image=image, verbose=False)
    #         res = out["pred"].detach().cpu().numpy()
    #
    #         for g, r, n in zip(gt, res, name):
    ...
    #
    #     accelerator.wait_for_everyone()
    #     avg_auc = accelerator.gather(torch.tensor(aucs).mean().to(device)).mean().item()
    #     _best_auc = max(_best_auc, avg_auc)
    #     return avg_auc, _best_auc
    ...
    #         image, gt, name, image_for_post = data['image'], data['gt'], data['name'], data['image_for_post']
    #         gt = [np.array(x, np.float32) for x in gt]
    #         gt = [x / x.max() + 1e-8 for x in gt]  # normalize ground truth
    #
    #         image = image.to(device).squeeze(1)
    #         ensem_out = self.train_val_forward_fn(model, image=image, time_ensemble=True,
    #                                               gt_sizes=[g.shape for g in gt], verbose=False)
    ...
    #
    #         # 转为 list 统一处理
    #         if isinstance(ensem_res, torch.Tensor):
    #             ensem_res = [ensem_res]
    #
    #         # 遍历处理每个预测结果
    ...
    #                 new_filename = filename + '.png'
    #                 plt.imsave(os.path.join(save_to, new_filename), r, cmap='gray', format='png')
    #
    #             auc = cal_auc(g, r)
    #             aucs.append(auc)
    #
    ...
    #             ensem_res = ensem_out["pred"].detach().cpu()
    #             batch_res.append(ensem_res)
    #         batch_res = torch.mean(torch.concat(batch_res, dim=1), dim=1, keepdim=True).numpy()
    #
    #         for g, r, n in zip(gt, batch_res, name):
    #             r = (r - r.min()) / (r.max() - r.min() + 1e-8)
    #             r = (r * 255).astype(np.uint8)
