...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def p_losses(self, x_start, times, cond_img, seg=None, extra_cond=None, noise=None, *args, **kwargs):
        ...
        model_out, pred_edge = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img)
        ...
        if self.loss_type == 'l2':
            ...
            # 边缘loss
            return self.loss_type(model_out, target, pred_edge=pred_edge)
    ...
    def p_mean_variance(self, x, cond, extra_cond, time, time_next):
        ...
        # 新增：
        pred = pred[0]
        # print('def p_mean_variance 中的 pred：', pred.shape)
