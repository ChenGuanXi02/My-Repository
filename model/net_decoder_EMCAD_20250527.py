...
from model.EMCAD_decoders import EMCAD

# 陈泽实验！灵感来源medsegdiff v1
import torch.fft
...
# 加个noise_feats
    def forward_features(self, x, timesteps, cond_img, noise_feats=None):
        ...
        if x1 is not None:
            x = self.fuse1(x1, f1)
        # print('F1F1F1F1F1F1F1F1F1F1F1F1F1F1F1F1F1F1F1F1')
        # print(x.shape)
        # #陈泽收尾
        ...
        if x3 is not None:
            x = self.fuse3(x3, f3)
        # print('F3F3F3F3F3F3F3F3F3F3F3F3F3F3F3')
        # print(x.shape)
        # #陈泽收尾
        ...
        if x4 is not None:
            x = self.fuse4(x4, f4)
        # print('F4F4F4F4F4F4F4F4F4F4F4F4F4F4F4')
        # print(x.shape)
        # #陈泽收尾
        outs.append(x) # F4
...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        self.decoder_EMCAD = EMCAD(channels= [512,320,128,64], kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu')
        self.up_EMCAD = nn.Sequential(
            # 空间上采样
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True),  # 或 F.interpolate(p1, scale_factor=4)

            # 通道调整
            ConvModule(in_channels=64, out_channels=32, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN', requires_grad=True)),
        )
        # [1,64,88,88] to [1,32,352,352]
    ...
    def forward(self, inputs, timesteps, x):
        ...
        # diffusion decoder -> EMCAD
        dec_outs = self.decoder_EMCAD(c4,[c3, c2, c1])
        # print(dec_outs[0].shape, dec_outs[1].shape, dec_outs[2].shape, dec_outs[3].shape)
        p1 = self.up_EMCAD(dec_outs[3]) # [1,64,88,88] to [1,32,352,352]
        # p4 = self.up_EMCAD(dec_outs[0])
        x = self.pred(p1)
...
class net(nn.Module):
    ...
    def forward(self, x, timesteps, cond_img):
        ...
        features, layer1, layer2, layer3, layer4 = self.decode_head(features, timesteps, noise_final)
        return features
