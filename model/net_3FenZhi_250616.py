...
class BDLU(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        # 最终下采样和特征增强
        x_l = self.final_lookup_downsample(lookup_ori)  # [B, 512, H_final, W_final]
...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        self.conv_block = Conv_Block(channels=128)
        ...
        # 新增轻量级融合模块
        self.feature_fuser = LightFeatureFuser(
            in_channels=[256, 256, 512, 512],  # 分别对应 x, _c, xf, xl
            target_channel=512,
            align_channel=256,
            use_se=True
        )
    ...
    def forward(self, inputs, timesteps, x):
        ...
        # 融合四个特征，送入到解码器中
        x = self.feature_fuser(x, _c, xf, xl)  # 输出: [B, 512, 88, 88]
        # print('解码器需要的特征尺寸', x.shape)
...
class LightFeatureFuser(nn.Module):
    def __init__(self,
                 in_channels: list,  # 输入特征的通道数列表，如 [256, 256, 512, 512]（对应x、_c、xf、xl）
                 target_channel: int = 512,  # 融合后输出的通道数
                 align_channel: int = 256,  # 对齐后的中间通道数（降低计算量）
                 use_se: bool = True):  # 是否启用SE注意力
        super().__init__()
        self.align_channel = align_channel
        self.target_channel = target_channel
        self.use_se = use_se

        # 1. 通道对齐：1x1卷积将不同通道的特征统一到align_channel
        self.align_convs = nn.ModuleList([
            nn.Conv2d(c, align_channel, kernel_size=1) for c in in_channels
        ])

        # 2. 融合后的注意力+降维模块
        self.fusion_block = nn.Sequential(
            nn.Conv2d(align_channel * 4, target_channel, kernel_size=3, padding=1),  # 拼接后4*align_channel→target_channel
        ...
        # 3. 可选SE注意力（轻量级通道加权）
        if use_se:
            self.se_block = SEBlock(target_channel, ratio=16)

    def forward(self, x, _c, xf, xl):
        """
        :param x: 特征1 (B, C1, H, W)
        :param _c: 特征2 (B, C2, H, W)
        :param xf: 特征3 (B, C3, H3, W3) （可能尺寸不同）
        :param xl: 特征4 (B, C4, H4, W4) （可能尺寸不同）
        :return: 融合特征 (B, target_channel, H, W)
        """
        # Step 1: 统一空间尺寸（以x的尺寸为基准）
        target_size = x.shape[2:]
        feats = [
            F.interpolate(x, size=target_size, mode='bilinear', align_corners=False),  # 若x已达标，无需插值
            F.interpolate(_c, size=target_size, mode='bilinear', align_corners=False),
            F.interpolate(xf, size=target_size, mode='bilinear', align_corners=False),
            F.interpolate(xl, size=target_size, mode='bilinear', align_corners=False)
        ]

        # Step 2: 通道对齐到align_channel
        aligned_feats = [
            self.align_convs[i](feats[i]) for i in range(len(feats))
        ]  # 每个特征变为 (B, align_channel, H, W)

        # Step 3: 拼接特征（通道维度）
        concat_feats = torch.cat(aligned_feats, dim=1)  # (B, 4*align_channel, H, W)

        # Step 4: 融合+降维
        fused = self.fusion_block(concat_feats)  # (B, target_channel, H, W)
