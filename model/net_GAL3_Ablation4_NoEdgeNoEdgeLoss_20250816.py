...
class Two_TFF(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(Two_TFF, self).__init__()
...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        self.Two_TFF = Two_TFF(in_channel=256, out_channel=256)
    ...
    def forward(self, inputs, timesteps, x):
        ...
        # 融合两分支（图像和频率）特征
        TwoXFusion = self.Two_TFF(_c, xf)

        # 融合后的特征与噪声特征x结合
        x = torch.cat([TwoXFusion, x], dim=1)
