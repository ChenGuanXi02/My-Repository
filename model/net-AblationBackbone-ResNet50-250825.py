...
# resnet基础残差块，适用于 ResNet-18、ResNet-34 等较浅的网络，结构简单
...
# 适用于 ResNet-50、101、152 等较深的网络，瓶颈残差块
...
class ResNet(nn.Module):
    def __init__(
            ...
            norm_layer: Optional[Callable[..., nn.Module]] = None,
            in_chans: int = 3,  # 新增参数：输入图像通道数，默认3
    ) -> None:
        super().__init__()
...
class resnet34(ResNet):
    def __init__(self, **kwargs):
        # 初始化ResNet34，使用BasicBlock，layers=[3,4,6,3]
        super().__init__(block=BasicBlock, layers=[3, 4, 6, 3], **kwargs)

        # 移除原分类头，用于特征提取
        self.fc = nn.Identity()

        # 修正时间嵌入模块的输入维度（统一为64，与timestep_embedding输出维度匹配）
        self.time_embeds = nn.ModuleList([
            nn.Sequential(
                nn.Linear(64, 4 * 64),  # 输入维度64
                nn.SiLU(),
                nn.Linear(4 * 64, 64)
            ),
            nn.Sequential(
                nn.Linear(64, 4 * 128),  # 修正：输入维度从128改为64
                nn.SiLU(),
                nn.Linear(4 * 128, 128)
            ),
            nn.Sequential(
                nn.Linear(64, 4 * 256),  # 修正：输入维度从256改为64
                nn.SiLU(),
                nn.Linear(4 * 256, 256)
            ),
            nn.Sequential(
                nn.Linear(64, 4 * 512),  # 修正：输入维度从512改为64
                nn.SiLU(),
                nn.Linear(4 * 512, 512)
            )
        ])

        # # 特征对齐卷积（确保与原PVT输出特征维度兼容）
        # self.align1 = nn.Conv2d(64, 64, kernel_size=1)  # layer1输出64→64
        # self.align2 = nn.Conv2d(128, 128, kernel_size=1)  # layer2输出128→128
        # self.align3 = nn.Conv2d(256, 256, kernel_size=1)  # layer3输出256→320
        # self.align4 = nn.Conv2d(512, 512, kernel_size=1)  # layer4输出512→512
    ...
    #     print(f"Loaded {len(matched_dict)}/{len(model_dict)} pretrained weights")

    def forward_features(self, x: Tensor, timesteps: Tensor, cond_img: Tensor, noise_feats=None):
        """
        适配原PVT的forward_features接口
        :param x: 噪声输入（此处兼容参数，实际使用cond_img作为输入）
        :param timesteps: 时间步
        :param cond_img: 条件图像
        :param noise_feats: 噪声特征（用于融合）
        """
        # 实际输入使用cond_img，保持接口兼容
        x = cond_img
        ...
        # 多尺度特征提取与时间嵌入融合
        feats = []
        time_emb = timestep_embedding(timesteps, 64)  # 基础时间嵌入

        # layer1输出（对应原PVT的stage1）
        x1 = self.layer1(x)
        te1 = self.time_embeds[0](time_emb).unsqueeze(-1).unsqueeze(-1)
        x1 = x1 + te1  # 融合时间嵌入
        # x1 = self.align1(x1)
        # print("x1.shape", x1.shape)
        feats.append(x1)

        # layer2输出（对应原PVT的stage2）
        x2 = self.layer2(x1)
        te2 = self.time_embeds[1](time_emb).unsqueeze(-1).unsqueeze(-1)
        x2 = x2 + te2
        # x2 = self.align2(x2)
        # print("x2.shape", x2.shape)
        feats.append(x2)

        # layer3输出（对应原PVT的stage3）
        x3 = self.layer3(x2)
        te3 = self.time_embeds[2](time_emb).unsqueeze(-1).unsqueeze(-1)
        x3 = x3 + te3
        # x3 = self.align3(x3)
        # print("x3.shape", x3.shape)
        feats.append(x3)

        # layer4输出（对应原PVT的stage4）
        x4 = self.layer4(x3)
        te4 = self.time_embeds[3](time_emb).unsqueeze(-1).unsqueeze(-1)
        x4 = x4 + te4
        # x4 = self.align4(x4)
        ...
        if noise_feats is not None:
            ...
            # 使用原有的FeatureAlignAndFuse模块
            # 创建fuse模块时，立即移动到输入数据所在设备（x1的设备）
            self.fuse1 = FeatureAlignAndFuse(
                x_channels=256, f_channels=64,
                target_h=x1.shape[2], target_w=x1.shape[3]
            ).to(x1.device)  # 关键：移动到x1所在设备（GPU）

            self.fuse2 = FeatureAlignAndFuse(
                x_channels=256, f_channels=128,
                target_h=x2.shape[2], target_w=x2.shape[3]
            ).to(x2.device)  # 移动到x2所在设备

            self.fuse3 = FeatureAlignAndFuse(
                x_channels=256, f_channels=256,
                target_h=x3.shape[2], target_w=x3.shape[3]
            ).to(x3.device)  # 移动到x3所在设备

            self.fuse4 = FeatureAlignAndFuse(
                x_channels=256, f_channels=512,
                target_h=x4.shape[2], target_w=x4.shape[3]
            ).to(x4.device)  # 移动到x4所在设备
...
class resnet50(ResNet):
    def __init__(self, **kwargs):
        super().__init__(block=Bottleneck, layers=[3, 4, 6, 3], **kwargs)
        self.fc = nn.Identity()  # 移除分类头
        # 保留时间嵌入模块（与原逻辑一致，用于特征-时间融合）
        self.time_embeds = nn.ModuleList([
            nn.Sequential(nn.Linear(64, 4 * 256), nn.SiLU(), nn.Linear(4 * 256, 256)),
            nn.Sequential(nn.Linear(64, 4 * 512), nn.SiLU(), nn.Linear(4 * 512, 512)),
            nn.Sequential(nn.Linear(64, 4 * 1024), nn.SiLU(), nn.Linear(4 * 1024, 1024)),
            nn.Sequential(nn.Linear(64, 4 * 2048), nn.SiLU(), nn.Linear(4 * 2048, 2048)),
        ])
    ...
    def forward_features(self, x, timesteps, cond_img, noise_feats=None):
        ...
        feats = []
        time_emb = timestep_embedding(timesteps, 64)

        # 输出原始通道特征（256, 512, 1024, 2048）
        x1 = self.layer1(x) + self.time_embeds[0](time_emb).unsqueeze(-1).unsqueeze(-1)
        feats.append(x1)  # [B, 256, H1, W1]

        x2 = self.layer2(x1) + self.time_embeds[1](time_emb).unsqueeze(-1).unsqueeze(-1)
        feats.append(x2)  # [B, 512, H2, W2]

        x3 = self.layer3(x2) + self.time_embeds[2](time_emb).unsqueeze(-1).unsqueeze(-1)
        feats.append(x3)  # [B, 1024, H3, W3]

        x4 = self.layer4(x3) + self.time_embeds[3](time_emb).unsqueeze(-1).unsqueeze(-1)
        feats.append(x4)  # [B, 2048, H4, W4]

        # 噪声特征融合（仅输出原始通道，不调整）
        if noise_feats is not None:
            n1, n2, n3, n4 = noise_feats
            self.fuse1 = FeatureAlignAndFuse(256, 256, x1.shape[2], x1.shape[3]).to(x1.device)
            self.fuse2 = FeatureAlignAndFuse(256, 512, x2.shape[2], x2.shape[3]).to(x2.device)
            self.fuse3 = FeatureAlignAndFuse(256, 1024, x3.shape[2], x3.shape[3]).to(x3.device)
            self.fuse4 = FeatureAlignAndFuse(256, 2048, x4.shape[2], x4.shape[3]).to(x4.device)
            a1, a2, a3, a4 = self.fuse1(n1, x1), self.fuse2(n2, x2), self.fuse3(n3, x3), self.fuse4(n4, x4)
            feats.extend([a1, a2, a3, a4])  # 原始通道：256, 512, 1024, 2048
        else:
            feats.extend([None, None, None, None])

        return feats  # 输出原始通道特征
...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        target_channels = (64, 128, 320, 512)

        # 新增：通道调整卷积（将 ResNet50 原始通道转为目标通道）
        self.adjust_c1 = nn.Conv2d(in_channels[0], target_channels[0], kernel_size=1)  # 256→64
        self.adjust_c2 = nn.Conv2d(in_channels[1], target_channels[1], kernel_size=1)  # 512→128
        self.adjust_c3 = nn.Conv2d(in_channels[2], target_channels[2], kernel_size=1)  # 1024→320
        self.adjust_c4 = nn.Conv2d(in_channels[3], target_channels[3], kernel_size=1)  # 2048→512

        # 噪声特征 a1-a4 的通道调整
        self.adjust_a1 = nn.Conv2d(in_channels[0], target_channels[0], kernel_size=1)  # 256→64
        self.adjust_a2 = nn.Conv2d(in_channels[1], target_channels[1], kernel_size=1)  # 512→128
        self.adjust_a3 = nn.Conv2d(in_channels[2], target_channels[2], kernel_size=1)  # 1024→320
        self.adjust_a4 = nn.Conv2d(in_channels[3], target_channels[3], kernel_size=1)  # 2048→512
    ...
    def forward(self, inputs, timesteps, x):
        ...
        # stage1:图像分支四个特征的融合
        # c.shape调整，为了resnet50的适配
        c1 = self.adjust_c1(c1)
        c2 = self.adjust_c2(c2)
        c3 = self.adjust_c3(c3)
        c4 = self.adjust_c4(c4)
        ...
        # stage2:频率噪声分支
        # a.shape调整
        a1 = self.adjust_a1(a1)
        a2 = self.adjust_a2(a2)
        a3 = self.adjust_a3(a3)
        a4 = self.adjust_a4(a4)
...
class net(nn.Module):
    def __init__(self, class_num=2, mask_chans=0, **kwargs):
        ...
        # backbone !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        self.backbone = resnet50(in_chans=3)
